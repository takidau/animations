\input{diagrams}
\def \XAxisToTen{1}
\def \ShowLateData{0}
\def \ShowAlignmentHelpers{0}
\def \FastRender{0}
\def \DrawDefaultLegend{0}
\def \oout{.3} % More transparency for outputs, since we're using bolder colors
\def \BackRectBorderWidth{2pt}
%\def \BackBorder{red}%c_back}
\def \DrawBackRect{1}
\def \OutputOverTimeline{1}
%\newcommand{\BendArrow}[1] {

\ifdefined \FirstStageOnly
\else
        \def \FirstStageOnly{0}
\fi

\newcommand{\sumA}[1]{
  \def \InputVersionSet{2}
  \def \InputVersion{1}
  \ResetMaxx
  \def \ox{0}
  \def \oy{\maxy / 2 + 1.5}
  \def \DefaultOutputBack{c_frances}
  \def \DiagramName{Frances}
  #1
  \if \FirstStageOnly 0
    \draw [->, s_watermark, color=c_back_light, line width=10pt] (\ox + 2.5 - .1666 / 2, \oy - 1) -- (\ox + 2.5 - .1666 / 2, \oy - 6) -- (\ox + 5 - .1666 / 2, \oy - 6);
  \fi
}

\newcommand{\sumB}[1]{
  \def \InputVersionSet{2}
  \def \InputVersion{2}
  \ResetMaxx
  %\def \ox{0}
  %\def \oy{-\maxy / 2 - 1}
  \def \ox{\maxx + 3.5}
  \def \oy{\maxy / 2 + 1.5}
  \def \DefaultOutputBack{c_tyler}
  \def \DiagramName{Tyler}
  #1
  \if \FirstStageOnly 0
    \draw [->, s_watermark, color=c_back_light, line width=10pt] (\ox + 7.5 - .333 + .1666 /2, \oy - 1) -- (\ox + 7.5 - .333 + .1666 / 2, \oy - 6) -- (\ox + 5 - .333 + .1666 / 2, \oy - 6);
  \fi
}
	
\newcommand{\topp}[1]{
\if \FirstStageOnly 0
  \def \InputVersionSet{2}
  \def \InputVersion{3}
  \ResetMaxx
  \def \ox{\maxx / 2 + 2 - .1666 - .1666 / 2}
  \def \oy{-\maxy / 2 - 1.5}
  \def \DefaultOutputBack{c_out_back}
  \def \DiagramName{Average Session Lengths}
  #1
  
  % Alignment helpers
  \if \ShowAlignmentHelpers 1
    \draw [-, s_watermark, color=white, line width=2pt] (\ox - 1.333, \oy + 2) -- (\ox - 1.333 - .5, \oy + 2);
    \draw [-, s_watermark, color=white, line width=2pt] (\ox + \maxx + 1 + .5, \oy + 2) -- (\ox + \maxx + 1, \oy + 2);

    \draw [-, s_watermark, color=white, line width=2pt] (\ox - 1.333, \oy + \maxy + 2) -- (\ox - 1.333 + 5, \oy + \maxy + 2);
    \draw [-, s_watermark, color=white, line width=2pt] (\ox + \maxx + 1 - 5, \oy + \maxy + 2) -- (\ox + \maxx + 1, \oy + \maxy + 2);
  \fi
\fi
}

\def \midx{\maxx + 1}
\def \midy{\maxy / 2}

\newcommand{\DrawOutputMarker}[2]{
	\node[circle,fill opacity=1,fill=c_yellow,draw opacity=1,draw=c_yellow,inner sep=2pt] at (\ox + #1, \oy + #2) {};
}

\newcommand{\junkfun}[6] {
        \def \MinX{#1}
        \def \MinY{#2}
        \def \MaxX{#3}
        \def \MaxY{#4}
        \def \CurY{#6}
        
        %\CurDim=\CurY pt
        %\MaxDim=\MaxY pt
        %\ifthenelse{\lengthtest{\CurDim > \MaxDim}}
         % {
        %  	\def \CurX{\MaxX}
	%	\def \CurY{\MaxY}
	 %}{
	 % }
        
	\DrawOutputMarker{\MinX}{\MinY}
	\draw [line width=1pt,c_yellow] (\ox + \MinX, \oy + \CurY) -- (\ox + \MinX, \oy + \MinY);
	\DrawInput{black,fill opacity=1}{\MaxX)}{\MaxY}{#5}{c_yellow,draw opacity=1,text opacity=1}
}

\newdimen \CurDim
\newdimen \MaxDim

\newcommand{\DrawMarkedOutput}[6]{
        \def \MinX{#1}
        \def \MinY{#2}
        %\def \MaxX{#3}
        \def \MaxX{#1}
        \def \MaxY{#4}
        \def \CurY{#6}
        \def \CurX{\MinX + ((\MaxX - \MinX) * (\CurY - \MinY) / (\MaxY - \MinY))}
        
        \CurDim=\CurY pt
        \MaxDim=\MaxY pt
        \ifthenelse{\lengthtest{\CurDim > \MaxDim}}
        {
          	\def \CurX{\MaxX}
		\def \CurY{\MaxY}
	}{}
        
	\DrawOutputMarker{\MinX}{\MinY}
	\draw [line width=1pt,c_yellow] ({\ox + \CurX}, \oy + \CurY) -- (\ox + \MinX, \oy + \MinY);
	\DrawInput{black,fill opacity=1}{\CurX}{\CurY}{#5}{c_yellow,draw opacity=1,text opacity=1}
	%\DrawInputNodeWonky{\InputX}{\InputY}{#4}{s_common}{rectangle, inner sep=4.5pt}
	%\node[fill=black,fill opacity=1,draw=c_yellow,draw opacity=1,text opacity=1,text=c_yellow,draw opacity=1,text opacity=1,font=\bf\sffamily,thick,c_yellow,draw opacity=1,text opacity=1] at (\ox + #1, \oy + #2) {#3};
}

\newcommand{\DrawStatex}[6]{
	\DrawState{#1}{#2}{#3}{#4}{#5}
	%\DrawInput{black!50!white}{#6}{#4}{#5}{\DefaultOutputBack!50!white}
}

\newdimen \startY
\newdimen \endY

\newcommand{\DrawIn}[4]{
  % #1 = current y
  % #2 = start y (inclusive)
  % #3 = end y (inclusive)
  % #4 = commands
  \currentY=#1 pt
  \startY=#2 pt
  \endY=#3 pt
  \ifthenelse{\lengthtest{\startY > \currentY}}
    {\def \AfterStart{0}}
    {\def \AfterStart{1}}
   \ifthenelse{\lengthtest{\currentY > \endY}}
    {\def \BeforeEnd{0}} % after
    {\def \BeforeEnd{1}}
    \if \AfterStart 1
    	\if \BeforeEnd 1
		#4
	\fi
    \fi
}

\newcommand{\Frame}[5][c_timeline] {
\TriggersMulti{
  %\draw [->, s_watermark, color=white] (\midx - 0.5, \midy - 2.5) -- (\midx + 1.25, \midy - .5);

  \sumA{\TriggersInline{#2}{a
  	\def \InputStyle{3}

	\DrawIn{#2}{0.333}{0.777}{\DrawStatex{0.333}{1.333}{0.000}{#2}{1.0}{1.333}}	
	\DrawAt{#2}{0.833}{
		\DrawOutputz{0.333}{1.333}{0.000}{0.833}{1.0}{on-time}
		%\DrawInput{black}{1.333}{0.866}{5}{c_watermark_e}
	}

	\DrawIn{#2}{0.666}{1.000}{\DrawStatex{1.555}{2.555}{0.000}{#2}{1.0}{2.555}}	
	\DrawAt{#2}{1.05}{
		\DrawOutputz{1.555}{2.555}{0.000}{1.05}{1.0}{on-time}
		%\DrawInput{black}{2.555}{1.05}{9}{c_watermark_e}
	}

	\DrawIn{#2}{0.777}{2.000}{\DrawStatex{2.777}{3.777}{0.000}{#2}{1.0}{3.777}}	

	\DrawIn{#2}{1.222}{1.333}{\DrawStatex{4.777}{5.777}{0.000}{#2}{1.0}{5.777}}	
	\DrawIn{#2}{1.444}{1.555}{\DrawStatex{3.999}{5.777}{0.000}{#2}{1.8}{5.777}}	
	\DrawIn{#2}{1.666}{2.000}{\DrawStatex{3.999}{5.777}{0.000}{#2}{1.8}{5.777}}	

	\DrawIn{#2}{2.111}{2.555}{\DrawStatex{2.777}{5.777}{0.000}{#2}{3.0}{5.777}}	
	
	\DrawAt{#2}{2.633}{
		\DrawOutputz{2.777}{5.777}{0.000}{2.666}{3.0}{on-time}
		%\DrawInput{black}{5.777}{2.633}{25}{c_watermark_e}
	}

	\DrawIn{#2}{2.333}{3.555}{\DrawStatex{6.666}{7.666}{0.000}{#2}{1.0}{7.666}}	
	\DrawIn{#2}{3.666}{3.888}{\DrawStatex{6.666}{8.444}{0.000}{#2}{1.8}{8.444}}	
	\DrawIn{#2}{3.666}{3.888}{\DrawStatex{6.666}{8.444}{0.000}{#2}{1.8}{8.444}}	
	\DrawIn{#2}{4.000}{4.444}{\DrawStatex{6.666}{8.777}{0.000}{#2}{2.1}{8.777}}	

	\DrawAt{#2}{4.533}{
		\DrawOutputz{6.666}{8.777}{0.000}{4.533}{2.1}{on-time}
		%\DrawInput{black}{8.777}{4.533}{12}{c_watermark_e}
	}

        #3
  }}
  \sumB{\TriggersInline{#2}{
  	\def \InputStyle{2}

	\DrawIn{#2}{0.3}{0.555}{\DrawStatex{1.999}{2.999}{0.000}{#2}{1.0}{2.999}}	
	\DrawIn{#2}{0.333}{0.555}{\DrawStatex{0.333}{1.333}{0.000}{#2}{1.0}{1.333}}	

	\DrawIn{#2}{0.666}{2.000}{\DrawStatex{0.333}{3.222}{0.000}{#2}{2.9}{3.222}}	
	\DrawIn{#2}{2.111}{2.444}{\DrawStatex{0.333}{3.888}{0.000}{#2}{3.5}{3.888}}	

	\DrawAt{#2}{2.5}{
		\DrawOutputz{0.333}{3.888}{0.000}{2.5}{3.5}{on-time}
	}

	\DrawIn{#2}{1.555}{2.555}{\DrawStatex{3.95}{4.95}{0.000}{#2}{1.0}{4.95}}	
	\DrawAt{#2}{2.6}{
		\DrawOutputz{3.95}{4.95}{0.000}{2.6}{1.0}{on-time}
	}

	\DrawIn{#2}{1.333}{2.111}{\DrawStatex{5.888}{6.888}{0.000}{#2}{1.0}{6.888}}	
	\DrawIn{#2}{2.222}{3.111}{\DrawStatex{5.5}{6.888}{0.000}{#2}{1.4}{6.888}}	
	\DrawAt{#2}{3.2}{
		\DrawOutputz{5.5}{6.888}{0.000}{3.2}{1.4}{on-time}
	}

	\DrawIn{#2}{2.333}{3.333}{\DrawStatex{7.000}{8.000}{0.000}{#2}{1.0}{8.000}}	
	\DrawIn{#2}{3.444}{3.888}{\DrawStatex{7.000}{8.444}{0.000}{#2}{1.4}{8.444}}	
	\DrawIn{#2}{4.000}{4.777}{\DrawStatex{7.000}{9.333}{0.000}{#2}{2.3}{9.333}}	
	\DrawAt{#2}{4.833}{
		\DrawOutputz{7.000}{9.333}{0.000}{4.833}{2.3}{on-time}
	}

	\DrawAt{#2}{2.5}{
		%\DrawInput{black}{3.888}{2.5}{26}{c_watermark_f}
	}

	\DrawAt{#2}{2.6}{
		%\DrawInput{black}{4.95}{2.6}{8}{c_watermark_f}
	}

	\DrawAt{#2}{3.2}{
		%\DrawInput{black}{6.888}{3.2}{8}{c_watermark_f}
	}

	\DrawAt{#2}{4.833}{
		%\DrawInput{black}{9.333}{4.833}{13}{c_watermark_f}
	}


        #4
  }}
  \topp{\TriggersInline{#2}{

	%\DrawInput{black}{1.444}{0.866}{5}{c_watermark_e}
	%\DrawInput{black}{5.333}{2.633}{25}{c_watermark_e}
	%\DrawInput{black}{8.777}{4.533}{12}{c_watermark_e}

	%\DrawInput{black}{3.888}{2.5}{26}{c_watermark_f}
	%\DrawInput{black}{4.95}{2.6}{8}{c_watermark_f}
	%\DrawInput{black}{6.888}{3.2}{8}{c_watermark_f}
	%\DrawInput{black}{9.333}{4.833}{13}{c_watermark_f}

        \def \InputStyle{1}
	%\DrawIn{#2}{0.866}{0.888}{\DrawStatex{0}{2}{0.000}{#2}{5}{2}}	
	\DrawIn{#2}{0.866}{0.888}{\DrawState{0}{2}{0.000}{#2}{1.0}{2}}	
    	\DrawAt{#2}{0.9}{
	    	\DrawOutputz{0}{2}{0.000}{0.9}{1.0}{on-time}
		%\DrawInput{black}{2}{0.9}{5}{c_yellow}
	}

	\DrawIn{#2}{1.05}{2.444}{\DrawStatex{2}{4}{0.000}{#2}{1.0}{4}}	
    	\DrawAt{#2}{2.5}{
	    	\DrawOutputz{2}{4}{0.000}{2.5}{2.25}{on-time}
		%\DrawInput{black}{4}{2.5}{35}{c_yellow}
	}
    	\DrawAt{#2}{2}{
	    	\DrawOutputz{2}{4}{0.000}{2}{1.0}{early}
		%\DrawInput{black}{4}{2}{9}{c_yellow}
	}

    	\DrawAt{#2}{2.666}{
		\DrawOutputz{4}{6}{0.000}{2.666}{2.0}{on-time}
		%\DrawInput{black}{6}{2.666}{33}{c_yellow}
	}

	\DrawIn{#2}{3.2}{4.333}{\DrawStatex{6}{8}{0.000}{#2}{1.0}{8}}	
    	\DrawAt{#2}{4}{
		\DrawOutputz{6}{8}{0.000}{4}{1.4}{on-time}
		%\DrawInput{black}{8}{4}{8}{c_yellow}
	}

	\DrawIn{#2}{4.533}{4.832}{\DrawStatex{8}{10}{0.000}{#2}{2.1}{10}}	
	\DrawIn{#2}{4.833}{5.111}{\DrawStatex{8}{10}{0.000}{#2}{2.2}{10}}	
    	\DrawAt{#2}{5.111}{
		\DrawOutputz{8}{10}{0.000}{5.111}{2.2}{on-time}
		%\DrawInput{black}{10}{5.111}{25}{c_yellow}
	}

  	#5
  }}
  
  \def \wmx{\maxx + .5}
  \def \wmy{-5.9}
  %\WatermarkLegend[0.5]{\wmx}{\wmy}{Mobile Scores - Input Watermark}{s_watermark,c_watermark_e}
  %\WatermarkLegend[0.5]{\wmx}{\wmy - .4}{Mobile Scores - Output Watermark}{s_output_watermark,c_watermark_e}
  %\WatermarkLegend[0.5]{\wmx}{\wmy - .8}{Console Scores - Input Watermark}{s_watermark,c_watermark_f}
  %\WatermarkLegend[0.5]{\wmx}{\wmy - 1.2}{Console Scores - Output Watermark}{s_output_watermark,c_watermark_f}
  %\WatermarkLegend[0.5]{\wmx}{\wmy - 1.6}{Top Teams - Input Watermark}{s_watermark,c_watermark_g,line width=2pt}
  %\WatermarkLegend[0.5]{\wmx}{\wmy - 2.0}{Top Teams - Output Watermark}{s_watermark,white,densely dotted,line width=2pt}

  % Already commented out before...  
  %\WatermarkLegend[0.5]{\wmx}{\wmy - 2.0}{Top Teams - Output Watermark}{s_output_watermark,c_datum_back}
}
}

\newcommand{\FrameA}[1] {
\Frame{#1}{}{}{}
}

\FrameA{.01}
\if \FastRender 0
\FrameA{.111}
\FrameA{.222}
\FrameA{.333}
\FrameA{.444}
\FrameA{.555}
\FrameA{.666}
\FrameA{.777}
\FrameA{.888}
\fi
\FrameA{1.000}
\if \FastRender 0
\FrameA{1.111}
\FrameA{1.222}
\FrameA{1.333}
\FrameA{1.444}
\FrameA{1.555}
\FrameA{1.666}
\FrameA{1.777}
\FrameA{1.888}
\FrameA{2.000}
\FrameA{2.111}
\FrameA{2.222}
\FrameA{2.333}
\FrameA{2.444}
\FrameA{2.555}
\FrameA{2.666}
\FrameA{2.777}
\FrameA{2.888}
\fi % \FastRender 0
\FrameA{3.000}
\if \FastRender 0
\FrameA{3.111}
\FrameA{3.222}
\FrameA{3.333}
\FrameA{3.444}
\FrameA{3.555}
\FrameA{3.666}
\FrameA{3.777}
\FrameA{3.888}
\FrameA{4.000}
\FrameA{4.111}
\FrameA{4.222}
\FrameA{4.333}
\FrameA{4.444}
\FrameA{4.555}
\FrameA{4.666}
\FrameA{4.777}
\FrameA{4.888}
\FrameA{5.000}
\fi % FastRender 0

\newcommand{\FrameV} {
\Frame[c_back]{5.222}{
  }{
  }{
  }
}
\FrameV
\if \FastRender 0
	\FrameV
	\FrameV
	\DrawFinalFrames{\FrameV}
\fi
\end{document}
