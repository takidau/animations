...
\newcommand{\DrawTemporalJoinWatermark}[1] {
		\draw [-, s_watermark, #1] 
			(\ox - 0.333, \oy + 0.01)
			to [out=80,in=210] (\ox - 0.333, \oy +  1.222) % 5
			to [out=30, in=195] (\ox + 3.4, \oy +  4.75) % 7
			%to [out=15, in=185] (\ox + 7.7, \oy +  5.2) % 1
			to [out=15, in=185] (\ox + 9.000, \oy +  5.3) % end
		;
}
...
\TriggersMulti{
	...
	\StreamInline{
		\DrawTemporalJoinWatermark{c_watermark}
	...
	\TableInline{
		...
		\DrawIn{\stYoff}{7.888}{15}{\DrawCurrency[3]{3}{3.666}{c_rate}{114 \textyen /\texteuro}} % New tail
		...
		\DrawIn{\stYoff}{6.333}{15}{\DrawCurrency[10]{6}{2.000}{c_rate}{114 \textyen /\texteuro}} % New tail
	...
	\def \stLabel{Yen}
	\StreamInline{
		% Draw the watermark in an unboosted scope so we don't draw over things in the previous section.
		\begin{scope}
		\clip (\stX, \stY) rectangle (\stXW, \stYH);
		\DrawTemporalJoinWatermark{c_watermark}
		\end{scope}

		\DrawIn{\stYoff}{8.111}{15}{\DrawCurrency{2}{3.86}{c_redo}{228 \texteuro}}

		\DrawIn{\stYoff}{9.333}{15}{\DrawCurrency{5}{5.065}{c_redo}{580 \textyen}}

		\DrawIn{\stYoff}{9.444}{15}{\DrawCurrency{7.75}{5.22}{c_redo}{357 \textyen}}
	}
...
\newcommand{\FastFrames}{100}
\newcommand{\AllFrames}{0,1,...,132}
